...
# ─── THE ONLY CHANGE FROM YOUR app.py ───────────────────────
# This tells Flask where to find your templates/ and static/
# folders, which are one level up from api/
...
_USER_INJECTION_PATTERNS = [
    ...
    r"<\s*/?prompt\s*>",
    r"\|{2,}",
    r"[A-Za-z0-9+/]{30,}={0,2}(?:\s|$)",
    r"(ignorer|ignorez|ignorar|игнорируй|無視して)",
...
def _build_responses(ticket: dict, greeting: str) -> dict:
    ...
    g   = greeting
    return {
        "order_status": (f"{g}I can see your order {oid} was placed {od} and is currently in **{st}** status. You should receive a shipping confirmation email within the next 1–2 business days. Our standard domestic shipping takes 5–7 business days once dispatched. Is there anything else I can help you with?"),
        "return_policy": (f"{g}Our return policy allows returns within **30 days** of delivery. Items must be unused and in their original packaging. If your items arrive damaged or incorrect, take photos and contact us straight away — we'll arrange a free return and send a replacement or full refund. Is there anything else I can help you with?"),
        "shipping": (f"{g}Standard domestic shipping takes **5–7 business days** after dispatch. Expedited shipping (2–3 days) is also available at checkout for future orders. Your order {oid} is currently {st} and will be dispatched shortly. Is there anything else I can help you with?"),
        "exchange": (f"{g}To exchange an item from order {oid}, log in to your ShopSmart account and go to **My Orders → Exchange Item**. You can select a different size, colour, or product. Exchanges are completely free within 30 days of delivery. Once we receive your return, the replacement will ship within 2–3 business days. Need help navigating the exchange process?"),
        "tracking": (f"{g}Your tracking number will be emailed to you as soon as order {oid} is dispatched. You can also find it any time by going to **My Orders → Track Shipment** in your ShopSmart account. Since your order is currently {st}, the tracking link will appear there once it leaves our warehouse. Is there anything else I can help with?"),
        "cancel": (f"{g}To cancel order {oid}, go to **My Orders → Cancel Order** in your ShopSmart account. Cancellations are free as long as the order hasn't shipped yet. Since your order is currently {st}, you should still be able to cancel it now. If it has already shipped, you can refuse delivery or use our 30-day return process. Would you like help with anything else?"),
        "payment_security": (f"{g}Your payment information is fully secure with ShopSmart. All transactions are protected with **256-bit SSL encryption**, and we never store your full card details — only a secure token via our PCI-DSS certified payment provider. You can review or remove saved payment methods anytime under **My Account → Payment Methods**. Is there anything else I can help with?"),
        "account_help": (f"{g}For account help: you can reset your password from the ShopSmart login page by clicking **'Forgot Password'**. To update your shipping address, go to **My Account → Addresses**. For further account issues, our team is reachable 24/7 at support@shopsmart.com. Is there anything else I can help you with?"),
        "discount_denied": (f"{g}Thank you for being a ShopSmart customer since {ms}! I'm not able to authorise discounts or special pricing — those are handled through our promotions team and email newsletter. Is there anything about your order or policies I can help with?"),
        "greeting": (f"{g}I'm the ShopSmart support assistant and I'm here to help! I can assist with order status, shipping, returns, exchanges, and account questions. What can I do for you today?"),
        "farewell": ("You're welcome — thank you for shopping with ShopSmart! If you ever need help again, don't hesitate to reach out. Have a wonderful day! 😊"),
        "default": (f"{g}I'm the ShopSmart support assistant. I can help with your order {oid} ({st}), shipping timeframes, our 30-day return policy, exchanges, and account questions. What would you like to know?"),
    }
...
def run_lab(untrusted: str, user_request: str, mode: str, is_first: bool = False):
    ...
    if mode == "vulnerable":
        if doc_override:
            ...
            explanation = ("🔴 Injection successful! A hidden LAB_OVERRIDE marker inside the customer ticket hijacked the response. In Vulnerable mode the system blindly executed it, generating a fake discount authorization. A real attacker could screenshot this and show it to a human agent. This is an indirect prompt injection attack.")
        elif user_injecting:
            assistant_text = _build_injection_success(ticket)
            explanation = ("🔴 Injection successful! The user message contained override instructions. In Vulnerable mode the chatbot followed them and issued a fake discount authorization. This is a direct prompt injection attack.")
        else:
        ...
        if injection_detected:
            ...
            explanation = ("🟢 Injection blocked. Defended mode detected the override attempt and refused to comply. The chatbot stayed in its ShopSmart assistant role.")
        else:
            assistant_text = _pick_response(user_request, ticket, is_first)
            explanation    = "No injection detected. The chatbot responded normally."
...
# DO NOT include app.run() — Vercel does not use it
